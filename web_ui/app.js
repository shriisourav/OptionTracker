...
// ============================================
// Chart - Shows ACTUAL option prices (not historical trends)
// ============================================
function updateChart() {
    ...
    const colors = {
        before: '#a855f7', below: '#a855f7',
        target: '#00d4ff',
        after: '#22c55e', above: '#22c55e'
    };

    // Build labels and data for bar chart
    const labels = series.map(s => {
        if (state.mode === 'strike') {
            return `$${s.data.strike}`;
        } else {
            return formatDateShort(s.date || s.label);
        }
    });
    ...
    const backgroundColors = series.map(s => {
        ...
        return s.position === 'target' ? base : base + '99'; // Add transparency for non-target
    });
    ...
    if (el.chartTitle) {
        const modeText = state.mode === 'strike' ? 'Strike' : 'Expiration';
        el.chartTitle.textContent = `Option Price by ${modeText} (Current Market Data)`;
    }
